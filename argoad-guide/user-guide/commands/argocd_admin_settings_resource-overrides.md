# `argocd admin settings resource-overrides` 命令參考
...
```
  -h, --help   resource-overrides 的說明
```

### 從父指令繼承的選項

```
      --argocd-cm-path 字串           本機 argocd-cm.yaml 檔案的路徑
      --argocd-context 字串           要使用的 Argo-CD 伺服器上下文名稱
      --argocd-secret-path 字串       本機 argocd-secret.yaml 檔案的路徑
      --as 字串                       模擬操作的使用者名稱
  ...
  -H, --header 字串陣列                  為 Argo CD CLI 發出的所有請求設定額外的標頭。(可以重複多次以新增多個標頭, 也支援以逗號分隔的標頭)
      ...
      --load-cluster-settings           表示應從叢集載入 config map 和 secret，除非提供本機檔案路徑
...
* [argocd admin settings](argocd_admin_settings.md)	 - 提供一組用於設定驗證和疑難排解的指令
* [argocd admin settings resource-overrides health](argocd_admin_settings_resource-overrides_health.md)	 - 評估資源健康狀態
* [argocd admin settings resource-overrides ignore-differences](argocd_admin_settings_resource-overrides_ignore-differences.md)	 - 呈現從差異比較中排除的欄位
* [argocd admin settings resource-overrides ignore-resource-updates](argocd_admin_settings_resource-overrides_ignore-resource-updates.md)	 - 呈現從資源更新中排除的欄位
* [argocd admin settings resource-overrides list-actions](argocd_admin_settings_resource-overrides_list-actions.md)	 - 列出可用的資源操作
* [argocd admin settings resource-overrides run-action](argocd_admin_settings_resource-overrides_run-action.md)	 - 執行資源操作
