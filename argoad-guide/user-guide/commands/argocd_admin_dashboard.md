# `argocd admin dashboard` 命令參考

## argocd admin dashboard

在本機啟動 Argo CD 網頁使用者介面

```
argocd admin dashboard [旗標]
```

### 範例

```
# 在本機預設的連接埠和位址上啟動 Argo CD 網頁使用者介面
$ argocd admin dashboard

# 在本機自訂的連接埠和位址上啟動 Argo CD 網頁使用者介面
$ argocd admin dashboard --port 8080 --address 127.0.0.1

# 使用 GZip 壓縮啟動 Argo CD 網頁使用者介面
...
```
      --address 字串                 在給定的位址上監聽 (預設為 "localhost")
      --as 字串                      模擬操作的使用者名稱
