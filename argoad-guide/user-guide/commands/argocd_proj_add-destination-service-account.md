# `argocd proj add-destination-service-account` 命令參考
...
```
  # 在名為 PROJECT 的專案上，為指定命名空間 (NAMESPACE) 中的伺服器 URL (SERVER) 新增專案目的地服務帳戶 (SERVICE_ACCOUNT)
  argocd proj add-destination-service-account PROJECT SERVER NAMESPACE SERVICE_ACCOUNT
  
  # 從不同的命名空間新增專案目的地服務帳戶 (SERVICE_ACCOUNT)
...
```
  -h, --help                               help for add-destination-service-account
      --service-account-namespace string   使用 service-account-namespace 作為服務帳戶所在的命名空間
```
