# `argocd admin settings validate` 命令參考
...
驗證 'argocd-cm' ConfigMap 和 'argocd-secret' Secret 中指定的設定
...
#驗證目前 kubeconfig 上下文的 Kubernetes 叢集中的帳戶和外掛程式設定
...
```
      --group 字串陣列   必須驗證的設定群組的選用清單 ( 可選：accounts, general, kustomize, resource-overrides)
  -h, --help                validate 的說明
```
