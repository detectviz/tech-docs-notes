# `argocd admin settings rbac validate` 命令參考
...
驗證 RBAC 策略的語法是否正確。策略必須是本機檔案或位於所提供命名空間中的 K8s ConfigMap，且格式為 CSV 或 K8s ConfigMap。
...
# 使用本機 policy.csv 檔案檢查給定的策略檔案是否有效。
...
# 如果未提供 --policy-file，而是提供了 --namespace，則會使用 K8s 中的 ConfigMap 'argocd-rbac-cm'。
argocd admin settings rbac validate --namespace argocd

# 必須提供 --policy-file 或 --namespace。
...
```
  ...
  -h, --help                           validate 的說明
      ...
      --namespace 字串               從中取得 argo rbac configmap 的命名空間
