# `argocd proj allow-cluster-resource` 命令參考

## argocd proj allow-cluster-resource

將叢集範圍的 API 資源新增至允許清單並從拒絕清單中移除
...
```
  # 將具有指定 GROUP 和 KIND 的叢集範圍 API 資源新增至專案 PROJECT 的允許清單，並從其拒絕清單中移除
...
```
  -h, --help          allow-cluster-resource 的說明
  -l, --list string   使用拒絕清單或允許清單。此選項只能是 'allow' 或 'deny' (預設為 "allow")
```
