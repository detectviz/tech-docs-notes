# `argocd admin export` 命令參考

## argocd admin export

將所有 Argo CD 資料匯出至 stdout (預設) 或檔案

```
argocd admin export [旗標]
```

### 選項

```
      --application-namespaces 字串      除了控制平面命名空間 (Argo CD 命名空間) 外，要從中匯出應用程式的命名空間 glob 的逗號分隔清單。預設情況下，一律會匯出控制平面命名空間中的所有應用程式。如果提供此旗標，則會連同控制平面命名空間一起匯出指定命名空間中的應用程式。如果未指定，則會使用 argocd-cmd-params-cm 中的 'application.namespaces' 值 (如果在 ConfigMap 中定義)。如果未設定 ConfigMap 值，則只會匯出控制平面命名空間中的應用程式。
      --applicationset-namespaces 字串   除了控制平面命名空間 (Argo CD 命名空間) 外，要從中匯出 ApplicationSets 的命名空間 glob 的逗號分隔清單。預設情況下，一律會匯出控制平面命名空間中的所有 ApplicationSets。如果提供此旗標，則會連同控制平面命名空間一起匯出指定命名空間中的 ApplicationSets。如果未指定，則會使用 argocd-cmd-params-cm 中的 'applicationsetcontroller.namespaces' 值 (如果在 ConfigMap 中定義)。如果未設定 ConfigMap 值，則只會匯出控制平面命名空間中的 ApplicationSets。
      --as 字串                           模擬操作的使用者名稱
  ...
  -o, --out 字串                          輸出至指定的檔案而非 stdout (預設為 "-")
