# `argocd proj add-destination` 命令參考
...
```
  # 在名為 PROJECT 的專案上，使用指定的命名空間 (NAMESPACE) 中的伺服器 URL (SERVER) 新增專案目的地
  argocd proj add-destination PROJECT SERVER NAMESPACE
  
  # 在名為 PROJECT 的專案上，使用指定的命名空間 (NAMESPACE) 中的伺服器名稱 (NAME) 新增專案目的地
...
```
  -h, --help   add-destination 的說明
      --name   使用名稱作為目的地，而非伺服器
```
