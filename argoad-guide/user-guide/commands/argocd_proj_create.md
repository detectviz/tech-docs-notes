...
```
  ...
  # 從檔案或 Kubernetes 清單的 URL 建立一個名為 PROJECT 的新專案
...
```
      ...
      --allow-namespaced-resource stringArray   允許的命名空間資源清單
      --deny-cluster-resource stringArray       拒絕的叢集層級資源清單
      --deny-namespaced-resource stringArray    拒絕的命名空間資源清單
      --description string                      專案描述
  -d, --dest stringArray                        允許的目的地伺服器和命名空間 (例如 https://192.168.99.100:8443,default)
      --dest-service-accounts stringArray       目的地伺服器、命名空間和目標服務帳戶 (例如 https://192.168.99.100:8443,default,default-sa)
  -f, --file string                             專案的 Kubernetes 清單的檔案名稱或 URL
  -h, --help                                    create 的說明
      --orphaned-resources                      啟用孤立資源監控
      --orphaned-resources-warn                 指定當偵測到孤立資源時，應用程式是否應有警告條件
      --signature-keys strings                  用於提交簽章驗證的 GnuPG 公鑰 ID
      --source-namespaces strings               應用程式的來源命名空間清單
  -s, --src stringArray                         允許的來源儲存庫 URL
      --upsert                                  即使提供的專案規格與現有規格不同，也允許覆寫同名專案
```
