...
包含一組對 Argo CD 管理員有用的指令，且需要直接存取 Kubernetes
...
```
  -h, --help               admin 的說明
      --logformat string   設定記錄格式。可選：json|text (預設為 "json")
      --loglevel string    設定記錄層級。可選：debug|info|warn|error (預設為 "info")
```
...
* [argocd admin dashboard](argocd_admin_dashboard.md)	 - 在本機啟動 Argo CD 網頁使用者介面
* [argocd admin export](argocd_admin_export.md)	 - 將所有 Argo CD 資料匯出至 stdout (預設) 或檔案
* [argocd admin import](argocd_admin_import.md)	 - 從 stdin (指定 `-') 或檔案匯入 Argo CD 資料
* [argocd admin initial-password](argocd_admin_initial-password.md)	 - 首次登入 Argo CD 時列印初始密碼
* [argocd admin notifications](argocd_admin_notifications.md)	 - 一組有助於管理通知設定的 CLI 指令
* [argocd admin proj](argocd_admin_proj.md)	 - 管理專案組態
* [argocd admin redis-initial-password](argocd_admin_redis-initial-password.md)	 - 確保 Redis 密碼存在，必要時建立新密碼。
* [argocd admin repo](argocd_admin_repo.md)	 - 管理儲存庫組態
