# `argocd account generate-token` 命令參考
...
```
argocd account generate-token [旗標]
```
...
```
  ...
  -e, --expires-in string   權杖過期前的持續時間。(預設：無過期時間) (預設為 "0s")
  -h, --help                generate-token 的說明
      --id string           可選的權杖 id。如果未指定值，則回退到 uuid。
```
