# `argocd app patch-resource` 命令參考
...
```
      --all                    表示是否修補多個符合的資源
      --group 字串           群組
  -h, --help                   patch-resource 的說明
      --kind 字串            種類
      --namespace 字串       命名空間
      --patch 字串           修補
      --patch-type 字串      要使用哪種修補策略：'application/json-patch+json'、'application/merge-patch+json' 或 'application/strategic-merge-patch+json'。預設為 'application/merge-patch+json' (預設為 "application/merge-patch+json")
      --project 字串         應用程式專案的名稱 - 指定此項可讓指令在應用程式不存在時回報「找不到」，而非「權限遭拒」
      --resource-name 字串   資源名稱
```
