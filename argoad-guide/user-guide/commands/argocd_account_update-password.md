# `argocd account update-password` 命令參考
...
此指令可用於更新目前登入使用者的密碼，
或在目前登入的使用者具有適當的 RBAC 權限以變更其他帳戶時，更新任意本機使用者帳戶。


```
argocd account update-password [旗標]
```
...
```
      --account string            應更新的帳戶名稱。預設為目前使用者帳戶
      --current-password string   目前登入使用者的密碼
  -h, --help                      update-password 的說明
      --new-password string       您要更新的新密碼
```
