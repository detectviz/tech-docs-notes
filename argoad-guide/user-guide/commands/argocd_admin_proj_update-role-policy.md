# `argocd admin proj update-role-policy` 命令參考

## argocd admin proj update-role-policy

實作大量專案角色更新。有助於回填現有的專案策略或移除過時的操作。
...
```
      ...
      --dry-run                        試運行 (預設為 true)
  -h, --help                           update-role-policy 的說明
  ...
  -n, --namespace 字串               如果存在, 此 CLI 請求的命名空間範圍
      ...
      --resource 字串                資源，例如 'applications'
      --role 字串                    角色名稱模式，例如 '*deployer*' (預設為 "*")
      --scope 字串                   資源範圍，例如 '*'
