# `argocd proj deny-namespace-resource` 命令參考

## argocd proj deny-namespace-resource

將命名空間範圍的 API 資源新增至拒絕清單，或從允許清單中移除命名空間範圍的 API 資源
...
```
  # 將具有指定 GROUP 和 KIND 的命名空間範圍 API 資源新增至專案 PROJECT 的拒絕清單，或從允許清單中移除
...
```
  -h, --help          deny-namespace-resource 的說明
