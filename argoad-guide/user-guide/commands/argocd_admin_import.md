# `argocd admin import` 命令參考

## argocd admin import

從 stdin (指定 `-') 或檔案匯入 Argo CD 資料
...
```
      --application-namespaces 字串      允許匯入應用程式的命名空間 glob 的逗號分隔清單。如果未提供, 將使用 argocd-cmd-params-cm 中的 'application.namespaces' 值。如果未定義, 則只有沒有明確命名空間的應用程式才會匯入到 Argo CD 命名空間
      --applicationset-namespaces 字串   允許匯入 applicationsets 的命名空間 glob 的逗號分隔清單。如果未提供, 將使用 argocd-cmd-params-cm 中的 'applicationsetcontroller.namespaces' 值。如果未定義, 則只有沒有明確命名空間的 applicationsets 才會匯入到 Argo CD 命名空間
      --as 字串                           模擬操作的使用者名稱
      ...
      --dry-run                             列印將要執行的操作
  -h, --help                                import 的說明
      --ignore-tracking                     如果資源已追蹤, 則不要更新追蹤註解
  ...
  -n, --namespace 字串                    如果存在, 此 CLI 請求的命名空間範圍
      --override-on-conflict                更新資源時, 如果發生衝突則覆寫資源
      ...
      --prune                               清理備份中未出現的密鑰、應用程式和專案
      --request-timeout 字串              在放棄單一伺服器請求之前等待的時間長度。非零值應包含對應的時間單位（例如 1s、2m、3h）。值為零表示請求不會逾時。(預設為 "0")
      --server 字串                       Kubernetes API 伺服器的位址和連接埠
      --skip-resources-with-label 字串    根據標籤略過匯入資源, 例如 '--skip-resources-with-label my-label/example.io=true'
      --stop-operation                      停止任何現有的操作
