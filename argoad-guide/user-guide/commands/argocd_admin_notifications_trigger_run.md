# `argocd admin notifications trigger run` 命令參考
...
# 執行設定在 'argocd-notification-cm' ConfigMap 中的觸發器
argocd admin notifications trigger run on-sync-status-unknown ./sample-app.yaml

# 使用 my-config-map.yaml 而非 'argocd-notifications-cm' ConfigMap 來執行觸發器
argocd admin notifications trigger run on-sync-status-unknown ./sample-app.yaml \
