...
```
  # 從檔案將 cd.example.com 的 TLS 憑證新增至 ArgoCD 憑證儲存庫
  argocd cert add-tls --from ~/mycert.pem cd.example.com

  # 透過 stdin 將 cd.example.com 的 TLS 憑證新增至 ArgoCD
  cat ~/mycert.pem | argocd cert add-tls cd.example.com

  # 透過掃描主機將 cd.example.com 的 SSH known host 項目新增至 ArgoCD
  ssh-keyscan cd.example.com | argocd cert add-ssh --batch

  # 列出所有已知的 TLS 憑證
  argocd cert list --cert-type https

  # 移除 cd.example.com 的所有 TLS 憑證
  argocd cert rm --cert-type https cd.example.com

  # 移除 cd.example.com 的所有憑證和 SSH known host 項目
...
* [argocd cert add-ssh](argocd_cert_add-ssh.md)	 - 為儲存庫伺服器新增 SSH known host 項目
* [argocd cert add-tls](argocd_cert_add-tls.md)	 - 為連線至儲存庫伺服器 SERVERNAME 新增 TLS 憑證資料
* [argocd cert list](argocd_cert_list.md)	 - 列出已設定的憑證
* [argocd cert rm](argocd_cert_rm.md)	 - 為 REPOSERVER 移除 TYPE 類型的憑證
