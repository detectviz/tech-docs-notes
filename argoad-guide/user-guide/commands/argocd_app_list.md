...
```
  ...
  # 按標籤列出應用程式，在此範例中，我們列出屬於另一個應用程式的子應用程式 (又稱應用程式的應用程式)
...
```
  -N, --app-namespace 字串   僅列出命名空間中的應用程式
  -c, --cluster 字串         按叢集名稱或 url 列出應用程式
  -h, --help                   list 的說明
  -o, --output 字串          輸出格式。可選：wide|name|json|yaml (預設為 "wide")
  -P, --path 字串            按路徑列出應用程式
  -p, --project 字串陣列    按專案名稱篩選
  -r, --repo 字串            按來源儲存庫 URL 列出應用程式
  -l, --selector 字串        按標籤列出應用程式。支援 '=', '==', '!=', in, notin, exists & not exists。相符的應用程式必須滿足所有指定的標籤限制。
```
