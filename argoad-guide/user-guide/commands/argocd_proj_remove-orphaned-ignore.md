# `argocd proj remove-orphaned-ignore` 命令參考
...
```
  # 從名為 PROJECT 的專案的孤立忽略清單中，移除指定 GROUP 和 KIND 的資源
  argocd proj remove-orphaned-ignore PROJECT GROUP KIND
  
  # 使用 NAME 模式從名為 PROJECT 的專案的孤立忽略清單中，移除指定 GROUP 和 KIND 的資源
...
```
  -h, --help          remove-orphaned-ignore 的說明
      --name string   資源名稱模式
```
