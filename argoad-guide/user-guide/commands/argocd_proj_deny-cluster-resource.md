# `argocd proj deny-cluster-resource` 命令參考

## argocd proj deny-cluster-resource

從允許清單中移除叢集範圍的 API 資源並將其新增至拒絕清單
...
```
  # 從專案 PROJECT 的允許清單中移除具有指定 GROUP 和 KIND 的叢集範圍 API 資源，並將其新增至拒絕清單
...
```
  -h, --help          deny-cluster-resource 的說明
