...
```
  # 將用戶端和伺服器的完整版本列印到標準輸出
  argocd version

  # 僅列印用戶端的完整版本 - 不會與伺服器建立連線
  argocd version --client

  # 以 JSON 格式列印用戶端和伺服器的完整版本
  argocd version -o json

  # 僅以 YAML 格式列印用戶端和伺服器的核心版本字串
...
```
  ...
  -o, --output 字串                  輸出格式。可選：json|yaml|wide|short (預設為 "wide")
