# `argocd proj add-orphaned-ignore` 命令參考
...
```
  # 將指定 GROUP 和 KIND 的資源新增至名為 PROJECT 的專案的孤立忽略清單
  argocd proj add-orphaned-ignore PROJECT GROUP KIND
  
  # 使用 NAME 模式將指定 GROUP 和 KIND 的資源新增至名為 PROJECT 的專案的孤立忽略清單
...
```
  -h, --help          add-orphaned-ignore 的說明
      --name string   資源名稱模式
```
