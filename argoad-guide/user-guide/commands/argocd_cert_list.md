...
```
      --cert-type 字串          僅列出給定類型的憑證，有效值：'ssh'、'https'
  -h, --help                      list 的說明
      --hostname-pattern 字串   僅列出符合給定 glob 模式的主機的憑證
  -o, --output 字串             輸出格式。可選：json|yaml|wide (預設為 "wide")
      --sort 字串               設定 wide 輸出格式的顯示排序順序。可選：hostname|type
```
...
* [argocd cert](argocd_cert.md)	 - 管理儲存庫憑證和 SSH known hosts 項目
