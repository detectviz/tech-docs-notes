...
```
      --applications 字串陣列   排程將套用至的應用程式。以逗號分隔，支援萬用字元 (例如 --applications prod-\*,website)
      --clusters 字串陣列       排程將套用至的叢集。以逗號分隔，支援萬用字元 (例如 --clusters prod,staging)
      --description 字串     同步視窗描述
      --duration 字串        同步視窗持續時間。 (例如 --duration 1h)
  -h, --help                   add 的說明
  -k, --kind 字串            同步視窗類型，可為 allow 或 deny
      --manual-sync            允許在 deny 和 allow 視窗中手動同步
      --namespaces 字串陣列     排程將套用至的命名空間。以逗號分隔，支援萬用字元 (例如 --namespaces default,\*-prod)
      --schedule 字串        同步視窗的 cron 格式排程。 (例如 --schedule "0 22 * * *")
      --time-zone 字串       同步視窗的時區 (預設為 "UTC")
      --use-and-operator       使用 AND 運算子來匹配應用程式、命名空間和叢集，而非預設的 OR 運算子
```
