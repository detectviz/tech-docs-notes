# `argocd proj remove-destination-service-account` 命令參考
...
```
  # 從名為 PROJECT 的專案上，自指定的目的地 (SERVER 和 NAMESPACE 組合) 中移除目的地服務帳戶 (SERVICE_ACCOUNT)
...
```
  -h, --help   remove-destination-service-account 的說明
```
