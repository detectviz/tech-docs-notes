# `argocd admin app get-reconcile-results` 命令參考

## argocd admin app get-reconcile-results

協調所有應用程式並將協調摘要儲存在指定的檔案中。

```
argocd admin app get-reconcile-results PATH [旗標]
```
...
```
  ...
  -h, --help                                              get-reconcile-results 的說明
      --ignore-normalizer-jq-execution-timeout duration   設定忽略正規化器 JQ 執行逾時 (預設 1s)
