# `argocd account get-user-info` 命令參考
...
```
argocd account get-user-info [旗標]
```

### 範例

```
  # 取得目前登入使用者的使用者資訊 (請參閱 'argocd login')
  argocd account get-user-info
...
```
  -h, --help            get-user-info 的說明
  -o, --output string   輸出格式。可選：yaml, json
```
