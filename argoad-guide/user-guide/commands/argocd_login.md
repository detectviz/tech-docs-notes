...
```
      --callback 字串      回呼 URL 的協定、主機和連接埠
  -h, --help                 login 的說明
      --name 字串          要用於上下文的名稱
      --password 字串      要驗證的帳戶密碼
      --skip-test-tls        略過測試伺服器是否設定了 TLS (這在指令無故掛起時會有幫助)
      --sso                  執行 SSO 登入
      --sso-launch-browser   執行 SSO 登入時自動啟動系統預設瀏覽器 (預設為 true)
      --sso-port 整數         執行本機 OAuth2 登入應用程式的連接埠 (預設為 8085)
      --username 字串      要驗證的帳戶使用者名稱
```
