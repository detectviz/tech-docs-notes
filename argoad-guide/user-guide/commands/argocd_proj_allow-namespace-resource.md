# `argocd proj allow-namespace-resource` 命令參考

## argocd proj allow-namespace-resource

從拒絕清單中移除命名空間範圍的 API 資源或將命名空間範圍的 API 資源新增至允許清單
...
```
  # 從專案 PROJECT 的拒絕清單中移除具有指定 GROUP 和 KIND 的命名空間範圍 API 資源，或將其新增至允許清單
...
```
  -h, --help          allow-namespace-resource 的說明
  -l, --list string   使用拒絕清單或允許清單。此選項只能是 'allow' 或 'deny' (預設為 "deny")
```
