# `argocd admin settings rbac can` 命令參考
...
檢查給定的角色或主體是否具有適當的 RBAC 權限來執行某項操作。
...
# 檢查角色 some:role 是否有權限在 'default' 專案中建立應用程式，
# 使用本機 policy.csv 檔案
argocd admin settings rbac can some:role create application 'default/app' --policy-file policy.csv

# 策略檔案也可以是 K8s 設定檔對應，其資料鍵類似 argocd-rbac-cm，
# 例如 'policy.csv' 和 (可選的) 'policy.default'
argocd admin settings rbac can some:role create application 'default/app' --policy-file argocd-rbac-cm.yaml

# 如果未提供 --policy-file，則會使用 K8s 中的 ConfigMap 'argocd-rbac-cm'。
# 您需要指定 argocd 命名空間，並確保您目前的 Kubernetes 上下文指向 Argo CD 執行所在的叢集
...
* [argocd admin settings rbac](argocd_admin_settings_rbac.md)	 - 驗證和測試 RBAC 設定
