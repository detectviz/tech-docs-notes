...
動作：[get create update delete sync override action invoke]
資源：[clusters projects applications applicationsets repositories write-repositories certificates accounts gpgkeys logs exec extensions]
...
```
      ...
      --core                            如果設定為 true，則 CLI 會直接與 Kubernetes 通訊，而不是與 Argo CD API 伺F服器通訊
  ...
  -H, --header 字串陣列                  設定 Argo CD CLI 發出的所有請求的附加標頭。(可以重複多次以新增多個標頭，也支援以逗號分隔的標頭)
      ...
      --server-name 字串              Name of the Argo CD API server; set this or the ARGOCD_SERVER_NAME environment variable when the server's name label differs from the default, for example when installing via the Helm chart (default "argocd-server")
```

### 另請參閱

* [argocd account](argocd_account.md)	 - 管理帳戶設定
