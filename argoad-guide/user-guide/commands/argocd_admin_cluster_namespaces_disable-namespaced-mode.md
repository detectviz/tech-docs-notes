# `argocd admin cluster namespaces disable-namespaced-mode` 命令參考

## argocd admin cluster namespaces disable-namespaced-mode

為名稱符合指定模式的叢集停用命名空間模式。

```
argocd admin cluster namespaces disable-namespaced-mode PATTERN [旗標]
```
...
```
  ...
  -h, --help                           disable-namespaced-mode 的說明
