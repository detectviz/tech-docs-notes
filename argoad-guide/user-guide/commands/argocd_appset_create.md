# `argocd appset create` 命令參考

## argocd appset create

建立一或多個 ApplicationSets
...
```
      --dry-run         允許在伺服器上評估 ApplicationSet 範本，以預覽將建立的應用程式
  -h, --help            create 的說明
  -o, --output string   輸出格式。可選：json|yaml|wide (預設為 "wide")
      --upsert          即使提供的 ApplicationSet 規格與現有規格不同，也允許覆寫同名的 ApplicationSet
```
