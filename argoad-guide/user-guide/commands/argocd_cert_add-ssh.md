# `argocd cert add-ssh` 命令參考

## argocd cert add-ssh

為儲存庫伺服器新增 SSH known host 項目
...
```
      --batch         透過讀取 SSH known hosts 資料來執行批次處理 (必要旗標)
      --from 字串   從檔案讀取 SSH known hosts 資料 (預設是從 stdin 讀取)
  -h, --help          add-ssh 的說明
      --upsert        如果輸入中的金鑰與現有的 SSH 伺服器公開主機金鑰不同，則取代現有的金鑰
```
