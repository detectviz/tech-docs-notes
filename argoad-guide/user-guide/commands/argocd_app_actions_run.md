# `argocd app actions run` 命令參考

## argocd app actions run

在符合指定篩選條件的資源上執行可用的操作。

### 摘要

除了 --kind 之外，所有篩選條件都是可選的。如果有多個資源符合篩選條件，請使用 --all 在所有符合的資源上執行操作。操作只能在 git 中表示的資源上執行，不能在子資源上執行。
...
```
      --all                    表示是否在多個符合的資源上執行操作
      --group 字串           應在其上執行操作的資源群組
  -h, --help                   run 的說明
      --kind 字串            應在其上執行操作的資源種類
      --namespace 字串       應在其上執行操作的資源命名空間
      --resource-name 字串   應在其上執行操作的資源名稱
```
