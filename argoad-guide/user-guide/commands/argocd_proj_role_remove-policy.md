# `argocd proj role remove-policy` 命令參考
...
1696759698  2023-10-08T11:08:18+01:00 (3 小時前)  <none>
...
```
  ...
  -h, --help                remove-policy 的說明
  -o, --object 字串       專案中要授予/拒絕存取的物件。使用 '*' 作為萬用字元。將會需要存取 '<專案>/<物件>'
  -p, --permission 字串   是否允許或拒絕使用該動作存取物件。只能是 'allow' 或 'deny' (預設為 "allow")
