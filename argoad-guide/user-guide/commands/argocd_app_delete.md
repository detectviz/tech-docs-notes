...
```
  -N, --app-namespace 字串        將從中刪除應用程式的命名空間
      --cascade                     執行所有應用程式資源的級聯刪除 (預設為 true)
  -h, --help                        delete 的說明
  -p, --propagation-policy 字串   指定應用程式資源刪除的傳播策略。可選：foreground|background (預設為 "foreground")
  -l, --selector 字串             刪除所有具有相符標籤的應用程式。支援 '=', '==', '!=', in, notin, exists & not exists。相符的應用程式必須滿足所有指定的標籤限制。
      --wait                        等到應用程式的刪除完成
  -y, --yes                         關閉提示以確認應用程式資源的級聯刪除
```
