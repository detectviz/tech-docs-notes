# `argocd admin settings resource-overrides run-action` 命令參考
...
使用 'argocd-cm' ConfigMap 中 'resource.customizations' 欄位設定的 lua 指令碼執行資源操作，並輸出更新的欄位
...
```
  -h, --help                run-action 的說明
      --param 字串陣列   操作參數 (例如 --param key1=value1)
```
