...
```
  ...
  # 按標籤等待應用程式，在此範例中，我們等待屬於另一個應用程式的子應用程式 (又稱應用程式的應用程式)
...
```
  -N, --app-namespace 字串   僅等待命名空間中的應用程式
      --degraded               等待降級
      --delete                 等待刪除
      --health                 等待健康
  -h, --help                   wait 的說明
      --hydrated               等待 hydrate 操作
      --operation              等待擱置操作
  ...
  -l, --selector 字串        按標籤等待應用程式。支援 '=', '==', '!=', in, notin, exists & not exists。相符的應用程式必須滿足所有指定的標籤限制。
      --suspended              等待暫停
      --sync                   等待同步
