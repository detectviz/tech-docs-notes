# `argocd admin app generate-spec` 命令參考
...
```
argocd admin app generate-spec APPNAME [旗標]
```
...
```
      --allow-empty                                為自動同步策略設定允許零個活動資源
      --annotations 字串陣列                    設定元資料註解 (例如 example=value)
      --auto-prune                                 為自動同步策略設定自動清理
      --config-management-plugin 字串            組態管理外掛程式名稱
      --dest-name 字串                           K8s 叢集名稱 (例如 minikube)
      --dest-namespace 字串                      K8s 目標命名空間
      --dest-server 字串                         K8s 叢集 URL (例如 https://kubernetes.default.svc)
      --directory-exclude 字串                   設定用於從應用程式來源路徑排除檔案的 glob 運算式
      --directory-include 字串                   設定用於從應用程式來源路徑包含檔案的 glob 運算式
      ...
      --env 字串                                 要監視的應用程式環境
  -f, --file 字串                                應用程式的 Kubernetes 資訊清單的檔案名稱或 URL
      --helm-api-versions 字串陣列              執行 helm template 時使用的 Helm api-versions (格式為 [group/]version/kind) (可以重複以設定多個值：--helm-api-versions traefik.io/v1alpha1/TLSOption --helm-api-versions v1/Service)。如果未設定，則使用目標叢集的 api-versions
      --helm-chart 字串                          Helm 圖表名稱
      --helm-kube-version 字串                   執行 helm template 時使用的 Helm kube-version。如果未設定，則使用目標叢集的 kube 版本
      ...
      --helm-set 字串陣列                       在命令列上設定 Helm 值 (可以重複以設定多個值：--helm-set key1=val1 --helm-set key2=val2)
      --helm-set-file 字串陣列                  從透過命令列指定的各個檔案設定 Helm 值 (可以重複以設定多個值：--helm-set-file key1=path1 --helm-set-file key2=path2)
      --helm-set-string 字串陣列                在命令列上設定 Helm STRING 值 (可以重複以設定多個值：--helm-set-string key1=val1 --helm-set-string key2=val2)
      --helm-skip-crds                             略過 helm crd 安裝步驟
      --helm-skip-schema-validation                略過 helm 結構描述驗證步驟
      --helm-skip-tests                            略過 helm 測試資訊清單安裝步驟
      --helm-version 字串                        Helm 版本
  -h, --help                                       generate-spec 的說明
      --hydrate-to-branch 字串                   要將應用程式渲染到的分支
      --ignore-missing-components                  設定 Kustomize 元件時忽略本機遺失的元件目錄
      --ignore-missing-value-files                 設定 helm template --values 時忽略本機遺失的 valueFiles
  -i, --inline                                     如果設定，則產生的資源會寫回 --file 旗標中指定的檔案
      --jsonnet-ext-var-code 字串陣列           Jsonnet ext var
      --jsonnet-ext-var-str 字串陣列            Jsonnet 字串 ext var
      --jsonnet-libs 字串陣列                   其他 jsonnet 函式庫 (以 repoRoot 為前綴)
      --jsonnet-tla-code 字串陣列               Jsonnet 頂層程式碼引數
      --jsonnet-tla-str 字串陣列                Jsonnet 頂層字串引數
      --kustomize-api-versions 字串陣列         執行 helm template 時使用的 api-versions (格式為 [group/]version/kind) (可以重複以設定多個值：--helm-api-versions traefik.io/v1alpha1/TLSOption --helm-api-versions v1/Service)。如果未設定，則使用目標叢集的 api-versions。僅在為 Kustomize 建置啟用 Helm 時適用
      --kustomize-common-annotation 字串陣列    在 Kustomize 中設定通用註解
      --kustomize-common-label 字串陣列         在 Kustomize 中設定通用標籤
      --kustomize-force-common-annotation          在 Kustomize 中強制通用註解
      --kustomize-force-common-label               在 Kustomize 中強制通用標籤
      ...
      --kustomize-kube-version 字串              執行 helm template 時使用的 kube-version。如果未設定，則使用目標叢集的 kube 版本。僅在為 Kustomize 建置啟用 Helm 時適用
      --kustomize-label-include-templates          將通用標籤套用至資源範本
      --kustomize-label-without-selector           不要將通用標籤套用至選取器。除非設定了 --kustomize-label-include-templates，否則也不要將標籤套用至範本
  ...
  -l, --label 字串陣列                          要套用至應用程式的標籤
      --name 字串                                應用程式的名稱，如果設定了檔案則忽略 (已棄用)
      --nameprefix 字串                          Kustomize nameprefix
      --namesuffix 字串                          Kustomize namesuffix
  ...
  -p, --parameter 字串陣列                      設定參數覆寫 (例如 -p guestbook=image=example/guestbook:latest)
      ...
      --revision-history-limit 整數                 要在修訂歷史記錄中保留的項目數 (預設為 10)
      --self-heal                                  為自動同步策略設定自我修復
      --set-finalizer                              在應用程式上設定刪除 finalizer，應用程式資源將在刪除時級聯
...
* [argocd admin app](argocd_admin_app.md)	 - 管理應用程式組態
