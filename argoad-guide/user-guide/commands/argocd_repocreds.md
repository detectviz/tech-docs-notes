...
```
      --as 字串                      模擬操作的使用者名稱
      --as-group stringArray           模擬操作的群組，此旗標可重複使用以指定多個群組。
      --as-uid 字串                  模擬操作的 UID
      ...
      --disable-compression            若為 true，則停用所有對伺服器請求的回應壓縮
  -h, --help                           repocreds 的說明
      --insecure-skip-tls-verify       若為 true，將不會檢查伺服器憑證的有效性。這會使您的 HTTPS 連線不安全
      --kubeconfig 字串              kube config 的路徑。僅在叢集外才需要
  -n, --namespace 字串               若存在，則為此 CLI 請求的命名空間範圍
      --password 字串                用於 API 伺服器基本驗證的密碼
      --proxy-url 字串               若提供，將使用此 URL 透過代理連線
      --request-timeout 字串         放棄單一伺服器請求前等待的時間長度。非零值應包含對應的時間單位 (例如 1s, 2m, 3h)。零值表示請求不會逾時。(預設為 "0")
      --tls-server-name 字串         若提供，此名稱將用於驗證伺服器憑證。若未提供，則使用聯繫伺服器的主機名稱。
      --token 字串                   用於 API 伺服器驗證的持有者權杖
      --user 字串                    要使用的 kubeconfig 使用者名稱
      --username 字串                用於 API 伺服器基本驗證的使用者名稱
```
...
* [argocd repocreds add](argocd_repocreds_add.md)	 - 新增 git 儲存庫連線參數
* [argocd repocreds list](argocd_repocreds_list.md)	 - 列出已設定的儲存庫憑證
* [argocd repocreds rm](argocd_repocreds_rm.md)	 - 移除儲存庫憑證
