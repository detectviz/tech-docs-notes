...
# 設定使用 Kubernetes API 伺服器的直接存取
argocd login cd.argoproj.io --core

# 如果使用者使用 "argocd login cd.argoproj.io" 並透過 SSO 登入
# "argocd relogin" 指令將會重新啟動 SSO 登入並更新伺服器上下文
```

### 選項

```
      --callback 字串      回呼 URL 的主機和連接埠
  -h, --help                 relogin 的說明
      --password 字串      用於驗證的帳戶密碼
      --sso-launch-browser   執行 SSO 登入時自動啟動預設瀏覽器 (預設為 true)
      --sso-port 整數         執行本機 OAuth2 登入應用程式的連接埠 (預設為 8085)
```
