# `argocd proj windows disable-manual-sync` 命令參考
...
停用同步視窗的手動同步功能。需要提供 ID，可透過執行 "argocd proj windows list PROJECT" 找到。
...
```
  -h, --help   disable-manual-sync 的說明
```
