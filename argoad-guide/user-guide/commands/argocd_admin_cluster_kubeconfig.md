# `argocd admin cluster kubeconfig` 命令參考
...
```
argocd admin cluster kubeconfig CLUSTER_URL OUTPUT_PATH [旗標]
```
...
#在控制台上為名為 "my-cluster" 的叢集產生一個 kubeconfig
argocd admin cluster kubeconfig my-cluster

#列出由 argocd 管理的叢集的可用 kubeconfig
argocd admin cluster kubeconfig

#移除特定的 kubeconfig 檔案
argocd admin cluster kubeconfig my-cluster --delete

#為叢集產生一個停用 TLS 驗證的 Kubeconfig
