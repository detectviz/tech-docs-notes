# `argocd proj windows enable-manual-sync` 命令參考
...
為同步視窗啟用手動同步。需要 ID，可以透過執行 "argocd proj windows list PROJECT" 找到。
...
# 為 ID 為 2 的預設專案上設定的視窗啟用手動同步
argocd proj windows enable-manual-sync default 2

# 使用自訂訊息啟用手動同步
argocd proj windows enable-manual-sync my-app-project --message "由管理員啟動的手動同步"
```

### 選項

```
  -h, --help   enable-manual-sync 的說明
```
