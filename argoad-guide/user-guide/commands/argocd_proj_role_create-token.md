# `argocd proj role create-token` 命令參考
...
```
  -e, --expires-in 字串   權杖過期前的持續時間，例如 "12h", "7d"。 (預設: 無過期時間)
  -h, --help                create-token 的說明
  -i, --id 字串           權杖唯一識別碼。 (預設: 隨機 UUID)
  -t, --token-only          僅輸出權杖 - 用於腳本中。
```
