# `argocd admin notifications` 命令參考
...
```
  ...
  -n, --namespace 字串                如果存在, 此 CLI 請求的命名空間範圍
      ...
      --tls-server-name 字串          如果提供, 此名稱將用於驗證伺服器憑證。如果未提供, 則使用用於聯絡伺F服器的主機名稱。
...
* [argocd admin notifications template](argocd_admin_notifications_template.md)	 - 通知範本相關指令
* [argocd admin notifications trigger](argocd_admin_notifications_trigger.md)	 - 通知觸發器相關指令
