...
本指南說明如何在面板插件中處理錯誤，並為常見情境提供建議。

## 提供可用的預設值

讓使用者可以循序漸進地學習您的面板插件。提供有用的預設設定，以便：
...
以下是一些您可能希望向使用者顯示錯誤的情況範例。

### 無效的查詢回應

使用者在為面板建立資料來源查詢時擁有完全的自由。如果您的面板插件需要特定格式的查詢回應，請使用面板畫布來引導使用者。

```ts
if (!numberField) {
  throw new Error('查詢結果缺少數字欄位');
}

if (frame.length === 0) {
  throw new Error('查詢傳回空結果');
}
```