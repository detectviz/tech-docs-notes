...
1. 在一個新目錄中，使用 create-plugin 工具從範本建立一個插件。當提示您選擇插件類型時，請選取 {props.pluginType} 並對「您是否希望您的插件有後端部分？」回答「是」：
...
7. 開啟 Grafana，預設為 [http://localhost:3000](http://localhost:3000)，然後前往 **管理** > **插件**。請確保您的 {props.pluginType} 插件在那裡。

您也可以透過檢查日誌來驗證 Grafana 是否已發現該插件：
